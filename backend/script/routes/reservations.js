...
router.get('/', async (req, res) => {
    ...
    if (id_reservation) {
        if (isNaN(id_reservation)) {
            return res.status(400).json({ message: 'id_reservation doit être un nombre.' });//erreur si l'id est pas un chiffre
        }
    ...
    if (time_rdv) filters.time_rdv = { contains: time_rdv, mode: 'insensitive' };// Recherche insensible à la casse
    if (id_service) {
        if (isNaN(id_service)) {
    ...
    if (id_client) {
        if (isNaN(id_client)) {
            return res.status(400).json({ message: 'id_client doit être un nombre.' }); //erreur si l'id est pas un chiffre
        }
        filters.id_client = parseInt(id_client);
    }
    if (id_prestataire) {
        if (isNaN(id_prestataire)) {
            return res.status(400).json({ message: 'id_prestataire doit être un nombre.' });//erreur si l'id est pas un chiffre
        }
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json(newReservation); // status 201 si requete reçu
    } catch (error) {
        console.error("Erreur lors de la création de la réservation:", error);
        res.status(500).json({ message: 'Erreur serveur', details: error.message }); //erreur 500 si probleme server
    }
});
